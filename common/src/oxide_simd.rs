...
impl SimdBatchProcessor {
    ...
    pub fn batch_ipv4_checksum(&self, headers: &[&[u8]]) -> Vec<u16> {
        ...
        self.stats.checksums_computed.fetch_add(headers.len() as u64, Ordering::Relaxed);
        results
    }
    ...
    pub fn batch_copy(&self, src: &[&[u8]], dst: &mut [&mut [u8]]) -> usize {
        ...
        self.stats.bytes_copied.fetch_add(total_bytes as u64, Ordering::Relaxed);
        self.stats.packets_processed.fetch_add(count as u64, Ordering::Relaxed);
        count
    }
    ...
    pub fn batch_parse_ipv4(&self, packets: &[&[u8]]) -> Vec<Ipv4Info> {
        ...
        self.stats.packets_processed.fetch_add(packets.len() as u64, Ordering::Relaxed);
        results
    }
...
mod tests {
    ...
    fn test_simd_detection() {
        ...
        println!("Detected SIMD: {:?}, width: {} bytes", cap, cap.vector_width());
