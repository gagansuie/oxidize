...
type ParsedHeader = (Option<IpAddr>, Option<IpAddr>, Option<u8>, Option<u16>, Option<u16>);
...
impl BufferPool {
    pub fn new() -> Self {
        ...
        Self {
            buffers,
            available: AtomicUsize::new((1 << BUFFER_POOL_SIZE) - 1), // All bits set
...
impl QueuedPacket {
    ...
    fn parse_ipv4_fast(data: &[u8], first_byte: u8) -> ParsedHeader {
        ...
        let src_bytes: [u8; 4] = unsafe {
            [*data.get_unchecked(12), *data.get_unchecked(13), 
             *data.get_unchecked(14), *data.get_unchecked(15)]
        };
        let dst_bytes: [u8; 4] = unsafe {
            [*data.get_unchecked(16), *data.get_unchecked(17),
             *data.get_unchecked(18), *data.get_unchecked(19)]
        };
        ...
        (Some(IpAddr::V4(src)), Some(IpAddr::V4(dst)), Some(protocol), src_port, dst_port)
    }
    ...
    fn parse_ipv6_fast(data: &[u8]) -> ParsedHeader {
        ...
        let (src_port, dst_port) = if has_ports {
            let sp = u16::from_be_bytes(unsafe {
                [*data.get_unchecked(40), *data.get_unchecked(41)]
            });
            let dp = u16::from_be_bytes(unsafe {
                [*data.get_unchecked(42), *data.get_unchecked(43)]
            });
            (Some(sp), Some(dp))
        } else {
            (None, None)
        ...
        (Some(IpAddr::V6(src)), Some(IpAddr::V6(dst)), Some(protocol), src_port, dst_port)
    }
...
pub trait PacketQueue: Send {
    ...
    fn set_verdict_modify(&mut self, packet_id: u32, verdict: Verdict, data: &[u8]) -> anyhow::Result<()>;
...
pub mod linux {
    ...
    impl NetlinkQueue {
        ...
        fn send_config_cmd(&mut self, cmd: u8, pf: u16) -> anyhow::Result<()> {
            ...
            let mut msg = self.build_nlmsghdr(total_len as u32, msg_type, NLM_F_REQUEST | NLM_F_ACK, seq);
        ...
        fn send_netlink(&self, data: &[u8]) -> anyhow::Result<()> {
            ...
            if sent < 0 {
                return Err(anyhow::anyhow!("sendto failed: {}", std::io::Error::last_os_error()));
            }

            Ok(())
        }

        fn recv_ack(&mut self, expected_seq: u32) -> anyhow::Result<()> {
            ...
            if n < 0 {
                return Err(anyhow::anyhow!("recv failed: {}", std::io::Error::last_os_error()));
            }
        ...
        fn parse_packet_msg(&self, data: &[u8]) -> anyhow::Result<QueuedPacket> {
            ...
            while pos + 4 <= data.len() {
                ...
                match attr_type {
                    NFQA_PACKET_HDR if attr_data.len() >= 7 => {
                        packet_id = u32::from_be_bytes([attr_data[0], attr_data[1], attr_data[2], attr_data[3]]);
                        hw_protocol = u16::from_be_bytes([attr_data[4], attr_data[5]]);
                        hook = attr_data[6];
                    }
                    NFQA_MARK if attr_data.len() >= 4 => {
                        mark = u32::from_be_bytes([attr_data[0], attr_data[1], attr_data[2], attr_data[3]]);
                    }
                    NFQA_PAYLOAD => {
            ...
            Ok(QueuedPacket::parse(packet_id, payload, hw_protocol, hook, mark))
        }
        ...
        fn setup_iptables(&self, config: &QueueConfig) -> anyhow::Result<()> {
            ...
            for port in &config.exclude_ports {
                let _ = Command::new("iptables")
                    .args(["-I", "OUTPUT", "-p", "udp", "--dport", &port.to_string(), "-j", "ACCEPT"])
                    .output();
            }
            ...
            if !output.status.success() {
                anyhow::bail!("Failed to set iptables rule: {}", 
                    String::from_utf8_lossy(&output.stderr));
            }
            
            tracing::info!("iptables NFQUEUE rules configured (queue {})", config.queue_num);
            Ok(())
        }
    ...
    impl PacketQueue for NetlinkQueue {
        ...
        fn recv(&mut self) -> anyhow::Result<QueuedPacket> {
            ...
            self.stats.bytes_received.fetch_add(packet.data.len() as u64, Ordering::Relaxed);
    ...
    impl PacketQueue for WinDivertQueue {
        fn bind(&mut self, config: QueueConfig) -> anyhow::Result<()> {
            ...
            let handle = WinDivert::network(
                ...
                0,  // priority
                WinDivertFlags::new(),
            ).map_err(|e| anyhow::anyhow!("Failed to open WinDivert: {:?} (requires admin + WinDivert driver)", e))?;
        ...
        fn recv(&mut self) -> anyhow::Result<QueuedPacket> {
            ...
            self.stats.packets_received.fetch_add(1, Ordering::Relaxed);
            self.stats.bytes_received.fetch_add(packet_len as u64, Ordering::Relaxed);
        ...
        fn set_verdict_modify(&mut self, _packet_id: u32, verdict: Verdict, data: &[u8]) -> anyhow::Result<()> {
...
pub mod macos {
    ...
    impl DivertQueue {
        ...
        fn setup_pf_rules(&self, config: &QueueConfig) -> anyhow::Result<()> {
            ...
            let pf_rules = format!(
                "pass out proto udp divert-to 127.0.0.1 port {}\n",
                port
            );
    ...
    impl PacketQueue for DivertQueue {
        fn bind(&mut self, config: QueueConfig) -> anyhow::Result<()> {
            ...
            ).map_err(|e| anyhow::anyhow!("Failed to create divert socket: {} (requires root)", e))?;
        ...
        fn recv(&mut self) -> anyhow::Result<QueuedPacket> {
            ...
            if n < 0 {
                self.stats.errors.fetch_add(1, Ordering::Relaxed);
                return Err(anyhow::anyhow!("recvfrom failed: {}", std::io::Error::last_os_error()));
            }
...
pub mod android {
    ...
    impl PacketQueue for VpnServiceQueue {
        ...
        fn recv(&mut self) -> anyhow::Result<QueuedPacket> {
            ...
            if n < 0 {
                self.stats.errors.fetch_add(1, Ordering::Relaxed);
                return Err(anyhow::anyhow!("read failed: {}", std::io::Error::last_os_error()));
            }
...
pub mod ios {
    ...
    impl PacketQueue for NetworkExtensionQueue {
        ...
        fn recv(&mut self) -> anyhow::Result<QueuedPacket> {
            ...
            self.stats.packets_received.fetch_add(1, Ordering::Relaxed);
            self.stats.bytes_received.fetch_add((n - 4) as u64, Ordering::Relaxed);
...
mod tests {
    ...
    fn test_packet_parsing_ipv4_udp() {
        ...
        assert_eq!(queued.src_ip, Some(IpAddr::V4(Ipv4Addr::new(192, 168, 1, 1))));
